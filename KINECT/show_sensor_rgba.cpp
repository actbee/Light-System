...
int main()
{
	...
	while (true)
	{
		...
		if (pFrameReader->AcquireLatestFrame(&pFrame) == S_OK)
		{
			...
			if (pFrame->CopyConvertedFrameDataToArray(uBufferSize, mImg.data, ColorImageFormat_Rgba) == S_OK)
			{
				cv::Mat show;
				cv::resize(mImg, show, cv::Size(iWidth/3, iHeight/3), 0, 0, cv::INTER_LINEAR);  //change the size to show
